...
class AmioApeInterface : public AmioInterfaceTemplate<WavpackReader, WavpackWriter>
{
protected:
	...
	virtual AmioResult OpenForRead(AmioOpenInterface& amioOpen)
	{
		...
		AmioApePrivateSettings privateSettings;
...
//              OutputDebugStringA (assign_string);
		...
		for (int metadataIndex = 0; metadataIndex < inReadFile.GetRiffMetadataItemCount(); metadataIndex++)
		{
				...
				(buffer[0] == '_'))
			{
				...
				// Here we strip the RIFF header as it appears in the Monkey's Audio file to obtain the size of the raw XMP metadata.
				size -= 8;
			...
			else
			{
				...
				//       ID because Monkey's Audio actually stores its metadata as RIFF chunks just like a Wave file.
				// NOTE: We mark all of these location hints as 1.0 since the APE file format has all the RIFF metadata at the end.
	...
	virtual AmioResult RunExportSettingsDialog(AmioFormatInterface& inFormat, void* inParentWindow, asdk::int64 inFlags, bool &outCancelled)
	{
		...
		AmioApeSettingsDialog dialog(privateSettings);
		outCancelled = true;
		if (dialog.RunModal(reinterpret_cast<AmioApeSettingsDialog::PlatformWindow>(inParentWindow)))
		{
			inFormat.SetPrivateFormatData(privateSettings.GetSerialized().c_str());
			outCancelled = false;
		}
