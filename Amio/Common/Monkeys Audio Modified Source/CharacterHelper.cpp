...
namespace 
{
	int GetUTF16Length(const str_utf16* pUTF16)
	{
		if (!pUTF16)
			return 0;
		
		int nCharacters = 0;
		const str_utf16* scan = pUTF16;
		while (*scan++ != 0)
			nCharacters++;
		return nCharacters;
	}
...
{
    const int nCharacters = GetUTF16Length(pUTF16);
    #ifdef _WIN32
...
{
    // get the size(s)
    int nCharacters = GetUTF16Length(pUTF16);
...
char * CAPECharacterHelper::GetCharacterBuffer(const char * pStr)
{
    int nCharacters = strlen(pStr);
    char* pOut = new char [nCharacters + 1];
	memcpy(pOut, pStr, nCharacters+1);
	return pOut;
}
