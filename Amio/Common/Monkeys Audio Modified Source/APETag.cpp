...
{
    // field name
    m_spFieldNameUTF16.Assign(new char [strlen(pFieldName) + 1], TRUE);
    memcpy(m_spFieldNameUTF16, pFieldName, (strlen(pFieldName) + 1) * sizeof(char));
...
{
    CSmartPtr<char> spFieldNameANSI(m_spFieldNameUTF16, TRUE); 
...
int CAPETag::GetTagFieldIndex(const char * pFieldName)
{
    ...
    for (int z = 0; z < m_nFields; z++)
    {
        if (stricmp(m_aryFields[z]->GetFieldName(), pFieldName) == 0)
            return z;
    }
...
int CAPETag::GetFieldString(const char * pFieldName, char * pBuffer, int * pBufferCharacters)
{
    if (m_bAnalyzed == FALSE) { Analyze(); }
    ...
    if (*pBufferCharacters > 0)
    {
        ...
        else if (pAPETagField->GetIsUTF8Text() || (m_nAPETagVersion < 2000))
        {
            ...
            spUTF16.Assign(CAPECharacterHelper::GetCharacterBuffer(pAPETagField->GetFieldValue()), TRUE);

            // get the number of characters
            int nCharacters = (strlen(spUTF16) + 1);
            ...
            else
            {
                // just copy in
                *pBufferCharacters = nCharacters;
                memcpy(pBuffer, spUTF16.GetPtr(), *pBufferCharacters * sizeof(char));
...
int CAPETag::CreateID3Tag(ID3_TAG * pID3Tag)
{
    ...
    // empty
    memset(pID3Tag, 0, ID3_TAG_BYTES);
...
int CAPETag::LoadField(const char * pBuffer, int nMaximumBytes, int * pBytes)
{
    ...
    CSmartPtr<char> spNameUTF8(new char [nNameCharacters + 1], TRUE);
    memcpy(spNameUTF8, &pBuffer[nLocation], (nNameCharacters + 1) * sizeof(char));
    nLocation += nNameCharacters + 1;
    CSmartPtr<char> spNameUTF16(spNameUTF8.GetPtr(), TRUE);
...
int CAPETag::SetFieldString(const char * pFieldName, const char * pFieldValue)
{
    // remove if empty
    ...
    CSmartPtr<char> spFieldValueUTF8(CAPECharacterHelper::GetCharacterBuffer(pFieldValue), TRUE);
    return SetFieldString(pFieldName, (const char *) spFieldValueUTF8.GetPtr(), TRUE);
}

int CAPETag::SetFieldString(const char * pFieldName, const char * pFieldValue, BOOL bAlreadyUTF8Encoded)
{
    // remove if empty
    if ((pFieldValue == NULL) || (strlen(pFieldValue) <= 0))
        return RemoveField(pFieldName);
...
int CAPETag::SetFieldBinary(const char * pFieldName, const void * pFieldValue, int nFieldBytes, int nFieldFlags)
{
    if (m_bAnalyzed == FALSE) { Analyze(); }
    if (pFieldName == NULL) return -1;
...
int CAPETag::GetFieldID3String(const char * pFieldName, char * pBuffer, int nBytes)
{
    ...
    CSmartPtr<char> spBufferANSI(CAPECharacterHelper::GetCharacterBuffer(cBuffer), TRUE);
