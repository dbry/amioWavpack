...
namespace
{
	...
	INT_PTR WINAPI ApeSettingsDialogProc(HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
	{
		switch (wMsg)
		{
		case  WM_INITDIALOG:
...
namespace amio
{
	...
	bool AmioApeSettingsDialog::RunModal(PlatformWindow inPlatformWindow)
	{
		...
		INT_PTR nResult = DialogBoxParam(amio::gModuleInstance,
								...
								inPlatformWindow,
								ApeSettingsDialogProc,
								NULL);
		if (IDOK == nResult)
		{
			mSettings.SetCompressionLevel(
					AmioApePrivateSettings::GetCompressionLevel(
					static_cast<AmioApePrivateSettings::CompressionIndex>(gRadioIndexCompressionLevel)));
			return true;
		}
		return false;
	}
